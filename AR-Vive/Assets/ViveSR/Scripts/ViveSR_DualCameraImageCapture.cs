...
namespace Vive.Plugin.SR
{
    ...
    public class ViveSR_DualCameraImageCapture
    {
        ...
        private static DataInfo[] DataInfoDistorted = new DataInfo[6];
        private static DataInfo[] DataInfoUndistorted = new DataInfo[6];
        private static DataInfo[] DataInfoDepth = new DataInfo[5];
...
#region Depth LinkMethod / Commands / Parameters
        ...
        {
            int result = ViveSR_Framework.ChangeModuleLinkStatus(ViveSR_Framework.MODULE_ID_SEETHROUGH, ViveSR_Framework.MODULE_ID_DEPTH, DepthProcessing ? (int)WorkLinkMethod.NONE : (int)WorkLinkMethod.ACTIVE);
            if (result == (int)Error.WORK) DepthProcessing = active;
            return result;
        }
        private static bool _DepthRefinement = false;
        public  static bool DepthRefinement { get { return _DepthRefinement; } set { EnableDepthRefinement(value); } }
        ...
        public static bool DepthEdgeEnhance { get { return _DepthEdgeEnhance; } set { EnableDepthEdgeEnhance(value); } }
...
#region Camera Commands / Parameters
        ...
        {
            IntPtr pointer = Marshal.AllocCoTaskMem(Marshal.SizeOf(paramInfo));
            int result = ViveSR_Framework.GetParameterStruct(ViveSR_Framework.MODULE_ID_SEETHROUGH, (int)item, ref pointer);
            paramInfo = (CameraQualityInfo)Marshal.PtrToStructure(pointer, typeof(CameraQualityInfo));
        ...
        {
            IntPtr pointer = Marshal.AllocCoTaskMem(Marshal.SizeOf(paramInfo));
            Marshal.StructureToPtr(paramInfo, pointer, true);
            int result = ViveSR_Framework.SetParameterStruct(ViveSR_Framework.MODULE_ID_SEETHROUGH, (int)item, pointer);
            Marshal.FreeCoTaskMem(pointer);
            return result;
        }
#endregion
